...
- **📍 Nhập địa chỉ thật + geocoding / real-address input**: nhập địa chỉ giao hàng (có **gợi ý tự động/autocomplete** để tránh gõ sai), app tự tìm tọa độ qua **OpenStreetMap (miễn phí, không cần API key)** — hoặc dùng tọa độ X/Y như cũ
- **🛣️ Khoảng cách & thời gian theo ĐƯỜNG THẬT (OSRM) / real road network**: khi nhập địa chỉ, app gọi **OSRM** (định tuyến trên OpenStreetMap, miễn phí, không cần thẻ/billing) để lấy **quãng đường lái xe và thời gian thực tế** theo mạng lưới đường, không phải đường chim bay. Nếu offline thì tự fallback sang khoảng cách great-circle × hệ số đường + tốc độ trung bình / real driving distances and durations via free OSRM, with a straight-line fallback
