...
- **📍 Nhập địa chỉ thật + geocoding / real-address input**: nhập địa chỉ giao hàng (có **gợi ý tự động/autocomplete** để tránh gõ sai), app tự tìm tọa độ qua **OpenStreetMap (miễn phí, không cần API key)**, tính khoảng cách (dặm/km, có hệ số đường thực) và thời gian di chuyển từ **tốc độ trung bình** — hoặc dùng tọa độ X/Y như cũ / type delivery addresses with autocomplete, geocoded via free OpenStreetMap, distances (mi/km) and travel times computed from an average speed
